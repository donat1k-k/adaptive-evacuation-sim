...
- [ ] **E1 — Каркас приложения.** Vite + React + TypeScript, структура `src/`, strict TS, ESLint. Без логики симулятора.
- [ ] **E2 — Модели данных (`src/models`).** Типы: среда, агент, сценарий, конфиг, результат, событие. Пассивные данные.
- [ ] **E3 — Headless simulation engine (`src/simulation`).** Тики, движение, разрешение конфликтов по seed, застревание, `T_max`. Без React.
